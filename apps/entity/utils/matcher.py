...
def filterEntsByRightNeightbor(ents_list, ent_name, word_list, neightbourPosition):
    ...
    return [
        ent for ent in ents_list if ent.label_ == ent_name and ent[0].nbor(neightbourPosition).text not in word_list
    ]
...
def use_matchers(nlp, matcher, doc):
    ...
    spans = filterEntsByLeftNeightbor(
        spans,
        "NUM",
        [
            "página",
    ...
    spans = filterEntsByLeftNeightbor(
        spans, "NUM", ["página", "pag", "p", "pág", "fs", "art", "arts", "inciso", "artículo", "artículos", "inc"], -2
    )
    spans = filterEntsByRightNeightbor(
        spans, "NUM", ["inc", "hs", "horas", "metros", "m", "gr", "grs", "gramos", "km", "kg", "cm"], 1
    )
    return spans
